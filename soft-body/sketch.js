...
function setup() {
  ...
  createP('Drag the mouse to move the blob.').style('color', '#ffffff');
...
function moveShape() {
  ...
  // Prevent center from going out of bounds
  centerX = constrain(centerX, radius, width - radius);
  centerY = constrain(centerY, radius, height - radius);

  if (centerX <= radius || centerX >= width - radius) accelX *= -1;
  if (centerY <= radius || centerY >= height - radius) accelY *= -1;
  ...
  // Move nodes and prevent them from going out of bounds
  for (let i = 0; i < nodes; i++) {
    nodeX[i] = nodeStartX[i] + sin(angle[i]) * (accelX * 2);
    nodeY[i] = nodeStartY[i] + sin(angle[i]) * (accelY * 2);

    // Constrain each node within the canvas
    nodeX[i] = constrain(nodeX[i], 0, width);
    nodeY[i] = constrain(nodeY[i], 0, height);
